...
class AppDynamicsJob(unittest.TestCase):
    ...
    def test_app_dynamics_job(self):
        ...
        driver.find_element(By.XPATH,"//input[@type='email']").send_keys("juan@perez.com")
        file_inputs = driver.find_elements(By.XPATH, "//input[@type='file']")
        file_inputs[0].send_keys(os.path.join(BASE_DIR, "fixtures", "documento.pdf"))
        for i in range(60):
            try:
                if self.is_element_present(By.XPATH, "//div[contains(text(),'archivo no permitido')]"): break
            except: pass
            time.sleep(1)
