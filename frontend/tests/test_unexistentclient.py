...
class AppDynamicsJob2(unittest.TestCase):
    def setUp(self):
    ...
    def test_app_dynamics_job(self):
        ...
        driver.find_element(By.XPATH,"//div[@id='app']/div/div/div/form/div/select").click()
        Select(driver.find_element(By.XPATH,"//div[@id='app']/div/div/div/form/div/select")).select_by_visible_text("Baja")
        ...
        for i in range(60):
            try:
                if self.is_element_present(By.XPATH, "//div[contains(text(),'cliente no existe')]"): break
            except: pass
            time.sleep(1)
